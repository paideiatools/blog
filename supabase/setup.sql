-- =============================================================================
-- Paideias Blog — full database setup (run once in the Supabase SQL Editor)
-- Consolidates every migration: schema, RLS, triggers, storage bucket, seeds.
-- Admin bootstrap: the first person to sign up with sahariar99@gmail.com (or any
-- allow-listed email) becomes an admin automatically.
-- =============================================================================
...
-- ---------- admin invites (allowlist for granting admin by email) ----------
...
-- Auto-create a profile on signup; grant admin to the bootstrap email or any
-- allow-listed email.
...
begin
  ...
  values (
    ...
    case
      ...
      when exists (
        select 1 from public.admin_invites where lower(email) = lower(new.email)
      ) then 'admin'
      else 'reader'
    end
  );
...
-- ---------- comments (guests allowed via guest_name) ----------
create table public.comments (
  ...
  guest_name text check (char_length(guest_name) between 1 and 80),
  is_hidden boolean not null default false,
  created_at timestamptz not null default now(),
  constraint comments_author_or_guest
...
create policy "Authors and admins can update comments"
  on public.comments for update using (author_id = auth.uid() or public.is_admin());
...
create policy "Admins update blog media"
  on storage.objects for update using (bucket_id = 'blog-media' and public.is_admin());
create policy "Admins delete blog media"
  on storage.objects for delete using (bucket_id = 'blog-media' and public.is_admin());
